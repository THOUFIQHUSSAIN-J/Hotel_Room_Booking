...
export const SearchItem = ({item}) => {
  return (
    <div className="searchItem border-2 border-neutral-300 p-[10px] rounded-md flex justify-between gap-[20px] mb-[20px]">
        <img src={item.photos[0]} alt="img1" className="w-[200px] h-[200px] object-cover"/>
        ...
        <div className="iDetails flex-[1] flex flex-col justify-between">
            {item.rating && <div className="hRating flex justify-between">
                <span className="font-semibold">Excellent</span>
                <button className="bg-blue-800 text-white p-1 font-bold rounded-lg">{item.rating}</button>
            </div>}
